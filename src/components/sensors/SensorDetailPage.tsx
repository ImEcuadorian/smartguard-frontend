...
export function SensorDetailPage({ sensorId }: { sensorId: string }) {
    ...
    return createRealtimeClient(session.accessToken, [
      {
        ...
        onMessage: (message) => {
          console.log("[Realtime] Nueva lectura recibida:", message.body);

          // Refresca sensor general: lastReadingAt / status.
          void queryClient.invalidateQueries({
            queryKey: ["sensors", sensorId],
          });

          // Refresca historial. También actualiza:
          ...
          // Actualiza listado principal de sensores.
          void queryClient.invalidateQueries({
            queryKey: ["sensors"],
          });
        },
  ...
  if (sensor.isLoading) {
    return <LoadingState label="Cargando sensor" />;
  }
        ...
        <section className="grid gap-6 xl:grid-cols-[0.9fr_1.4fr]">
          <Card>
            ...
            <CardContent className="space-y-4 text-sm">
              <Info
                  label="Estado"
                  value={<StatusBadge status={sensor.data.status} />}
              />
              ...
              <Info
                  label="Ubicación"
                  value={sensor.data.location ?? "Sin ubicación"}
              />
          ...
          <Card className="xl:col-span-2">
            ...
            <CardContent>
              {!readings.isLoading && (readings.data?.length ?? 0) === 0 ? (
                  <p className="text-sm text-slate-400">
                    Aún no existen lecturas registradas para este sensor.
                  </p>
              ) : null}
...
function RuleForm({
  ...
  const [type, setType] =
      useState<SensorAlertRuleType>("NUMERIC_THRESHOLD");

  const [operator, setOperator] =
      useState<ComparisonOperator>("GREATER_THAN");

  const [thresholdValue, setThresholdValue] = useState("");
  const [expectedBooleanValue, setExpectedBooleanValue] = useState("true");
  const [durationMinutes, setDurationMinutes] = useState("");
  const [alertType, setAlertType] =
      useState<AlertType>("THRESHOLD_EXCEEDED");
      ...
      <form className="grid gap-4 sm:grid-cols-2" onSubmit={handleSubmit}>
        <div>
          ...
          <Select
              ...
              value={type}
              onChange={(event) =>
                  setType(event.target.value as SensorAlertRuleType)
              }
          >
        ...
        {type === "BOOLEAN_MATCH" ? (
            <div>
              ...
              <Select
                  ...
                  onChange={(event) =>
                      setExpectedBooleanValue(event.target.value)
                  }
              >
